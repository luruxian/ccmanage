...
class Package(Base):
    ...
    daily_reset_credits = Column(Integer, default=10000, nullable=False, comment=f"每日重置积分数，{PackageType.STANDARD}类型订阅默认为10000，{PackageType.MAX_SERIES}类型订阅默认为15000，{PackageType.EXPERIENCE_PACKAGE}和{PackageType.TEMPORARY_PACKAGE}类型订阅为0（不参与每日重置），{PackageType.FUEL_PACK}类型订阅为0")
    duration_days = Column(Integer, nullable=False, comment="订阅时长（天）")
    package_type = Column(String(2), default=PackageType.STANDARD, nullable=False, comment=f"订阅类型：{PackageType.STANDARD}-标准订阅，{PackageType.MAX_SERIES}-Max系列订阅，{PackageType.EXPERIENCE_PACKAGE}-体验积分包（固定积分10000，用完为止，不参与每日重置），{PackageType.TEMPORARY_PACKAGE}-临时积分包（固定积分5000，用完为止，不参与每日重置），{PackageType.FUEL_PACK}-加油包（只累加积分）")
